"""GBP/USD (CS.D.GBPUSD.TODAY.IP) instrument configuration for IG bot.

S5 SMC strategy on GBP/USD spot CFD.

IG epic verified 2026-05-19: price quoted in pip-scaled units
(e.g. 15230.1 means 1.52301). 1 pip = 1.0 in IG's price feed.
minDealSize=0.04, minNormalStopOrLimitDistance=4.0 pips (wider than EUR/USD).
"""
...
CONFIG = {
    ...
    # IG quotes GBP/USD in pip-scaled units to 0.1-pip precision (e.g. 15230.1).
    # Min stop distance per /markets API: 4 pips = 4.0 in IG units.
    "price_decimals":       1,
    "min_deal_distance":    4.0,
    ...
    # to match forex's ~0.8% daily range vs indices' ~1.5%; GBP slightly wider than EUR)
    "s5_enabled":                    True,
    ...
    "s5_ltf_interval":               "15m",
    "s5_ob_lookback":                40,
    "s5_ob_min_impulse":             0.0015,   # was 0.005  (~19 pips on GBP/USD)
    "s5_ob_min_range_pct":           0.00025,  # was 0.001  (~3 pips OB range)
    "s5_choch_lookback":             10,
    "s5_max_entry_buffer":           0.0025,   # was 0.01   (~31 pips above OB)
    "s5_sl_buffer_pct":              0.0005,   # was 0.002  (~6 pips below OB)
    "s5_ob_invalidation_buffer_pct": 0.00025,  # was 0.001  (~3 pips)
    ...
    "s5_min_rr":                     1,
    "s5_trail_range_pct":            1.25,     # was 5      (5% = ~625 pips, way wide)
    ...
    "s1_consolidation_range_pct": 0.0003,   # PLACEHOLDER — backtest will tune
    "s1_breakout_buffer_pct":     5e-05,   # PLACEHOLDER — backtest will tune
    "s1_atr_period":            14,
