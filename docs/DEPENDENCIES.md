...
- `strategies/s8.py` — evaluate_s8 (post-S2 bounce: tri-confluence of pre-flight base high + daily 20MA + 61.8% fib; big momentum candle clears a pre-flight base, no coil required; queue_pending, handle_pending_tick, exits)
...
- **Trade DNA trend fingerprint:** snap_trend_daily_*, snap_trend_h1_*, snap_trend_m15_*, snap_trend_m3_* (see trade_dna.py). Populated at entry by `dna_snapshot(strategy, symbol, candles)`; `candles` is assembled by `bot._dna_candles()`, which refetches whatever timeframes a strategy's `dna_fields()` needs but the (JSON-serialized, DataFrame-less) pending payload couldn't carry. `trade_dna._get_handler` dispatches S1–S7 (S7 added 2026-06-06).
...
**Note:** After scale-in, exit handling is split into two phases (`bot.py`, shared by Bitget/Bybit/Binance):
1. **Placement** — `_do_scale_in` places the scale-in market order once, sets `ap["scale_in_refresh_pending"]=True` (+ `scale_in_pre_qty`, `scale_in_refresh_deadline`, persisted via `st.update_position_memory`), clears `scale_in_pending`, then calls `_refresh_scale_in_exits` inline as a fast path.
2. **Refresh** — `_refresh_scale_in_exits(sym, ap)` runs while `scale_in_refresh_pending` is set (inline + every main-loop tick via the `elif` at the scale-in check). It waits until live position qty exceeds `scale_in_pre_qty` (fill confirmed) before fetching the new avg entry, recomputing trail trigger + SL, and calling `update_position_sl()` + `refresh_plan_exits()`. While unconfirmed it returns without touching exits (retry next tick) up to `scale_in_refresh_deadline` (`_SCALE_IN_REFRESH_TIMEOUT`=300s); past the deadline it refreshes with available data. It NEVER re-places the scale-in market order. Prevents sizing exits against the stale pre-scale qty when the REST position API lags (AMDUSDT bug, 2026-06-03). Flags are restored from `position_memory` on startup so a mid-window restart still completes the refresh.

For S2, `update_position_sl()` is called with `max(box_low * 0.999, new_avg * (1 - S2_STOP_LOSS_PCT))` so the SL trigger reflects the scaled-in avg entry price.
...
- 2026-06-03: S7 scale-in exit-sizing fix (AMDUSDT) — two bugs. (A) On scale-in fill lag, `bot._do_scale_in` polled ≤12s then fell through and refreshed `profit_plan`/`moving_plan` against the STALE pre-scale qty (left them at ¼ of the final position), marking scale-in complete with no retry. Fix: split into placement + refresh phases — `_do_scale_in` places the order once, sets `ap["scale_in_refresh_pending"]` (+ `scale_in_pre_qty`, `scale_in_refresh_deadline`, persisted to `position_memory` and restored on startup), then `_refresh_scale_in_exits(sym, ap)` (inline fast-path + `elif` in the main-loop scale-in check) waits for live qty > pre_qty before refreshing, retrying each tick up to `_SCALE_IN_REFRESH_TIMEOUT`=300s, never re-ordering. Shared by Bitget/Bybit/Binance. (B) Bitget-only: the size-bound preset `loss_plan` from the entry order was never cancelled when scale-in asserted the position-level `pos_loss`, leaving a stale half-size SL. Fix: `trader.update_position_sl` now calls new `trader._cancel_stale_loss_plan(symbol, hold_side)` after placing `pos_loss` (queries `orders-plan-pending` planType=profit_loss, cancels `loss_plan` for that posSide; leaves profit_plan/moving_plan/other-side untouched; best-effort). bybit/binance `update_position_sl` unchanged (no loss_plan). No CSV/return-value changes; additive `ap`/position_memory fields only. Out of scope: `rest_qty = round_qty(total-half)` float-tick loss (separate follow-up). Tests: tests/test_bot_scale_in_exits.py (defer/retry-no-reorder/keeps-pending); tests/test_trader_exits.py::TestUpdatePositionSLCancelsLossPlan. Docs: S7.md "After Scale-in", GENERAL_CONCEPTS.md §11, §6.1 above.
- 2026-06-03: round_qty tick-loss fix (follow-up to the scale-in fix above) — `rest_qty = round_qty(total - half)` dropped a tick when float error made the remainder land just under a step boundary (e.g. `0.09 - 0.04 = 0.04999999999999996` → `floor(4.999…)=4` → `0.04` instead of `0.05`), leaving part of a small-tick position uncovered by partial-TP/trailing. Fix: `math.floor(qty/step + 1e-9) * step` in all four rounders — `trader._round_qty`, `bitget.round_qty`, `bybit.round_qty`, `binance.round_qty` (the `+1e-9` absorbs representation error without changing genuine flooring; e.g. 4.5 still floors to 4). `backtest_engine._round_qty` uses `round()` and was unaffected. No signature change (alias parity intact). Makes code match the §2.1 claim that rest_qty "covers true remainder". Tests: new tests/test_round_qty_split.py asserts half+rest==total across all four rounders for small-tick totals.
- 2026-06-06: DNA trend-fingerprint population fix — `snap_trend_*` columns were almost entirely blank (audit: only S1's `snap_trend_daily_*` ever populated, 76/76; S2–S7 0%; `snap_trend_h1_*` 0% across all). Two bugs. (A) `trade_dna._get_handler` had no `S7` branch → `dna_snapshot("S7", …)` logged "unknown strategy S7" and returned `{}`, so ALL S7 trend fields were silently dropped. Fix: added `elif strategy == "S7": from strategies.s7 import dna_fields`. (B) S2–S7 fire through the pending watcher; pending signals are JSON-serialized to `state.json`, which can't carry the `daily_df`/`m15_df` DataFrames, so at fire time `candles["daily"]` was empty and every `dna_fields()` early-returned before computing anything; the H1 fields were never passed at any site. Fix: new `bot._dna_candles(symbol, strategy, *, daily=, h1=, m15=, m3=)` assembles the candles dict, reusing in-scope DataFrames and refetching the rest via `bot._fetch_df` (intervals/limits in `_DNA_TIMEFRAMES`/`_DNA_FETCH`; daily=1D/100, h1=1H/48, m15=15m/100; m3 never fetched — S1-only, always in scope). All seven `dna_snapshot(...)` call sites in `bot.py` (S1/S2/S3/S4/S5×2/S6/S7 + 3 S5 watcher sites) now route through it. S7 fetches its 1H once and reuses it for both the fingerprint and the open snapshot. Adds a few `tr.get_candles` calls per entry (entries are rare; each fetch is guarded and degrades to "" on failure — never blocks a trade). Shared `bot.py` → affects Bitget/Bybit/Binance; ig_bot.py unaffected. No CSV columns added (snap_trend_* already in the §4.2 contract, just now non-blank), no state/config/return-value/signature changes. Tests: new tests/test_dna_h1_population.py (parametrized `_dna_candles` fetch coverage S1–S7, in-scope reuse, `_fire_s7` passes non-empty h1 → snap_trend_h1_ema_slope populated). Updated §4.2 fingerprint bullet.
- 2026-06-05: Bitget plan-trigger precision fix (HUSDT `4c326063`, 2026-06-04: S4 SHORT liquidated at −99.44%) — `bitget.place_profit_plan` and `bitget.place_moving_plan` posted `"triggerPrice": str(trigger)` without rounding to the symbol's price tick (`price_place`). After scale-in, `refresh_plan_exits` cancelled the existing `profit_plan`/`moving_plan`, then tried to re-place them with `trail_trigger = new_avg * (1 − S4_TRAILING_TRIGGER_PCT) = 0.55776 × 0.9 = 0.5019798103443…` (13 decimals). Bitget rejected all 3 retries with `HTTP 400 [40808] checkBDScale error value=0.5019798103443 checkScale=5`, logging `⚠️ Scale-in exits refresh failed — verify plan orders manually`. The doubled position was then naked of partial-TP and trailing; only the SL preset remained, and when a market-wide altcoin pump gapped HUSDT through it at 2026-06-05 07:42 UTC the position auto-liquidated. Fix: `place_profit_plan` and `place_moving_plan` in `bitget.py` now wrap `trigger` with `round_price(trigger, symbol)`, matching the existing pattern in `bybit.place_profit_plan` / `bybit.place_moving_plan` (lines 502/562) and `binance.place_profit_plan` / `binance.place_moving_plan` (lines 543/585). No signature change. Bybit and Binance were already correct; Bitget is now in parity. No state.json/CSV/return-value changes. Tests: new tests/test_bitget_plan_trigger_rounding.py — three tests covering (a) unrounded 13-decimal trigger gets rounded to 5 decimals for `place_profit_plan`, (b) same for `place_moving_plan`, (c) already-tick-aligned value is idempotent (no last-digit drift).
