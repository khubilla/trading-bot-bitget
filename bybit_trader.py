"""
bybit_trader.py — Bybit USDT-Perp high-level trader.

Mirrors the public surface of trader.py so bybit_bot.py can be a structural
clone of bot.py. open_long() / open_short() honour DRY_RUN from config_bybit.

When `config_bybit.DRY_RUN` is True, open_long/open_short LOG the intended
order and return a simulated fill dict without hitting /v5/order/create.

Strategy modules are reused unchanged. bybit_bot.py installs sys.modules
aliases at startup (`config_s1` → `config_bybit_s1`, `bitget` → `bybit`,
`trader` → `bybit_trader`) so when strategies do `from config_s1 import X`
or `import bitget as bg` they transparently get Bybit equivalents.
"""
...
import bybit as bb
from config_bybit import CATEGORY, SETTLE_COIN, DRY_RUN
from config_bybit_s1 import (
...
# ── Symbol info / rounding — delegate to bybit module ───────────── #

def _sym_info(symbol: str) -> dict:
    return bb.sym_info(symbol)
...
def get_candles(symbol: str, interval: str, limit: int = 100) -> pd.DataFrame:
    return bb.get_candles(symbol, interval, limit)
...
def get_usdt_balance() -> float:
    return bb.get_usdt_balance()


def _get_total_equity() -> float:
    return bb.get_total_equity()


def get_all_open_positions() -> dict[str, dict]:
    return bb.get_all_open_positions()


def get_realized_pnl(symbol: str, retries: int = 3, retry_delay: float = 1.5) -> float | None:
    return bb.get_realized_pnl(symbol, retries=retries, retry_delay=retry_delay)
...
def get_history_position(symbol: str,
    ...
    return bb.get_history_position(symbol, open_time_iso, entry_price, retries, retry_delay)
...
def set_leverage(symbol: str, leverage: int):
    if DRY_RUN:
        logger.info(f"[Bybit][DRY_RUN][{symbol}] set_leverage({leverage}x) — skipped")
        return
    bb.set_leverage(symbol, leverage)
...
def _place_tpsl(symbol: str, hold_side: str,
    ...
    if DRY_RUN:
        logger.info(f"[Bybit][DRY_RUN][{symbol}] place TP/SL hold={hold_side} "
                    f"TP={tp_trig:.5f} SL={sl_trig:.5f}")
        return True
    return bb.place_pos_tpsl_full(symbol, hold_side, tp_trig, tp_exec, sl_trig, sl_exec)


def update_position_sl(symbol: str, new_sl: float, hold_side: str = "long") -> bool:
    if DRY_RUN:
        logger.info(f"[Bybit][DRY_RUN][{symbol}] update_position_sl hold={hold_side} → {new_sl:.5f}")
        return True
    return bb.update_position_sl(symbol, new_sl, hold_side)


def cancel_all_orders(symbol: str):
    if DRY_RUN:
        logger.info(f"[Bybit][DRY_RUN][{symbol}] cancel_all_orders — skipped")
        return
...
# aliasing installed by bybit_bot.py at startup redirects those imports
# to `bybit` / `bybit_trader`, so the strategy code runs unchanged.
...
def open_long(
    ...
    logger.info(f"[Bybit][{symbol}] 📤 Market BUY: qty={qty} @ mark≈{mark:.5f} "
                f"| preset SL={sl_trig_preset:.5f}")

    if DRY_RUN:
        logger.info(f"[Bybit][DRY_RUN][{symbol}] open_long would call /v5/order/create")
        fill = mark
    else:
        bb.place_market_order(symbol, "buy", qty, sl_trigger=sl_trig_preset)
        _t.sleep(2.0)
    ...
    if not ok:
        logger.error(f"[Bybit][{symbol}] ⚠️  TP/SL failed! Set manually: SL={sl_trig}")
    ...
    logger.info(
        f"[Bybit][{symbol}] 🟢 LONG {leverage}x | qty={qty} entry≈{fill:.5f} "
...
def open_short(
    ...
    logger.info(f"[Bybit][{symbol}] 📤 Market SELL: qty={qty} @ mark≈{mark:.5f} "
                f"| preset SL={sl_trig:.5f}")

    if DRY_RUN:
        logger.info(f"[Bybit][DRY_RUN][{symbol}] open_short would call /v5/order/create")
        fill = mark
    else:
        bb.place_market_order(symbol, "sell", qty, sl_trigger=sl_trig)
        _t.sleep(2.0)
        _pos_after = get_all_open_positions()
        fill = _pos_after.get(symbol, {}).get("entry_price", 0) or mark
    ...
    if not ok:
        logger.error(f"[Bybit][{symbol}] ⚠️  TP/SL failed! Set manually: SL={sl_trig} TP={tp_trig}")
    ...
    logger.info(
        f"[Bybit][{symbol}] 🔴 SHORT {leverage}x | qty={qty} entry≈{fill:.5f} "
        f"SL={sl_trig} TP={tp_trig} | {'✅' if ok else '❌ SET MANUALLY'}"
    )
    return result
...
def scale_in_long(symbol: str, additional_trade_size_pct: float, leverage: int) -> None:
    ...
    if DRY_RUN:
        ...
        return
    bb.place_market_order(symbol, "buy", qty)
    logger.info(f"[Bybit][{symbol}] ➕ Scale-in LONG qty={qty} @ mark≈{mark:.5f}")
...
def scale_in_short(symbol: str, additional_trade_size_pct: float, leverage: int) -> None:
    ...
    if DRY_RUN:
        ...
        return
    bb.place_market_order(symbol, "sell", qty)
    logger.info(f"[Bybit][{symbol}] ➕ Scale-in SHORT qty={qty} @ mark≈{mark:.5f}")
...
def place_limit_long(symbol: str, limit_price: float, sl_price: float,
    ...
    if DRY_RUN:
        ...
        logger.info(f"[Bybit][DRY_RUN][{symbol}] place_limit_long → {fake_id}")
        return fake_id
    return bb.place_plan_order("buy", symbol, limit_price, sl_price, tp_price, qty_str)
...
def place_limit_short(symbol: str, limit_price: float, sl_price: float,
    ...
    if DRY_RUN:
        ...
        logger.info(f"[Bybit][DRY_RUN][{symbol}] place_limit_short → {fake_id}")
        return fake_id
    return bb.place_plan_order("sell", symbol, limit_price, sl_price, tp_price, qty_str)


def cancel_order(symbol: str, order_id: str) -> None:
    if DRY_RUN:
        logger.info(f"[Bybit][DRY_RUN][{symbol}] cancel_order {order_id}")
        return
    bb.cancel_plan_order(symbol, order_id)


def get_order_fill(symbol: str, order_id: str) -> dict:
    if DRY_RUN and str(order_id).startswith("DRY-"):
        # In DRY_RUN, simulate the conditional remaining live indefinitely.
        return {"status": "live", "fill_price": 0.0}
    return bb.get_order_fill(symbol, order_id)
