"""EUR/USD (CS.D.EURUSD.TODAY.IP) instrument configuration for IG bot.

S5 SMC strategy on EUR/USD spot CFD.

IG epic verified 2026-05-19: price quoted in pip-scaled units
(e.g. 13050.9 means 1.30509). 1 pip = 1.0 in IG's price feed.
...
CONFIG = {
    ...
    # IG quotes EUR/USD in pip-scaled units to 0.1-pip precision (e.g. 13050.9).
    ...
    # to match forex's ~0.6% daily range vs indices' ~1.5%)
    "s5_enabled":                    True,
    ...
    "s5_ltf_interval":               "15m",
    "s5_ob_lookback":                40,
    "s5_ob_min_impulse":             0.0015,   # was 0.005  (~16 pips on EUR/USD)
    "s5_ob_min_range_pct":           0.00025,  # was 0.001  (~3 pips OB range)
    "s5_choch_lookback":             10,
    "s5_max_entry_buffer":           0.0025,   # was 0.01   (~27 pips above OB)
    "s5_sl_buffer_pct":              0.0005,   # was 0.002  (~5 pips below OB)
    ...
    "s5_min_rr":                     1,
    "s5_trail_range_pct":            1.25,     # was 5      (5% = ~540 pips, way wide)
